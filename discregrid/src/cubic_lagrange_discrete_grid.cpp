...
unsigned int CubicLagrangeDiscreteGrid::addFunction(std::vector<std::pair<Eigen::Vector3d,double*>>& locs, bool verbose,
														SamplePredicate const &pred)
{
	using namespace std::chrono;
...
// #pragma omp for schedule(static) nowait
	for (int l = 0; l < static_cast<int>(n_nodes); ++l)
	{
		...
		if (verbose &&
			(++counter == n_nodes || duration_cast<milliseconds>(high_resolution_clock::now() - t0).count() > 1000u))
		{
			std::async(std::launch::async, [&]()
			{
				mutex.lock();
	...
	if (verbose)
	{
		std::cout << "\rConstruction took " << std::setw(15) << static_cast<double>(duration_cast<milliseconds>(
			high_resolution_clock::now() - t0_construction).count()) / 1000.0 << "s" << std::endl;
	}
